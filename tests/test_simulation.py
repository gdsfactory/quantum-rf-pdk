...
def test_transmon_worker_builds_a_connected_palace_mesh(tmp_path):
    _require_gsim()
    sweep = importlib.import_module("qpdk.simulation.transmon_sweep")
    assert callable(sweep.evaluate_layout)

    sim_dir = sweep.build_simulation(
        sweep.Layout(pad_width=20, pad_height=40, pad_gap=10),
        tmp_path / "trial",
        num_modes=1,
