...
1. Bump the version with `uv version --bump patch` The version should be updated in `pyproject.toml`,
   `qpdk/__init__.py`, and `README.md`
...
- **Component tests**: Each new component should be added to the cell registry with `@gf.cell` and categorized with
  `tags`.
...
1. Categorize the component using the `tags` parameter in the `@gf.cell` decorator (e.g.,
...
1. Add regression tests following the existing parametrized test pattern, generated by running `just test-gds-force`
   multiple times
...
1. Run `just setup-ipython-config` to set up IPython configuration
