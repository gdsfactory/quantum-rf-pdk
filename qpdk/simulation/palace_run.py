...
def palace_command(*, ranks: int) -> list[str]:
    ...
    """
    command = ["mpirun", "-np", str(ranks), "palace", "config.json"]
    if sif := os.environ.get("QPDK_PALACE_SIF"):
...
def evaluate_simulation(sim_dir: Path) -> dict[str, Any]:
    ...
    Returns:
        A row of results: ``f01`` in Hz, ``T1`` in seconds, the quality factor,
        the junction participation, and the per-domain electric-energy
        participations.

    Note:
        ``f01`` is the *linearised* mode frequency, which is what an eigenmode
        solve of a junction-as-inductor model returns. The real 0-1 transition
    ...
    # ~1e-6 a spurious mode shows and far below the ~0.3 a junction mode does.
    ...
    f01 = float(eig[index]["Re{f} (GHz)"]) * 1e9
    quality_factor = float(eig[index]["Q"])
    ...
    row: dict[str, Any] = {
        ...
        "T1": quality_factor / (2.0 * np.pi * f01),
    ...
    for key, value in domains[index].items():
        if key.startswith("p_elec["):
            row[f"participation_domain{key[len('p_elec[') : -1]}"] = float(value)
    return row
