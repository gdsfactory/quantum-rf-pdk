...
- **One job per trial**, submitted as the study goes. Each asks for a few ranks
  and a memory limit on each requested node. The driver keeps a fixed number
  in flight.
...
class SlurmCluster:
    ...
    Attributes:
        ...
        log_dir: Directory for Slurm's job output. Relative paths are resolved
            when the script is rendered.
        solver_cores: Cores one solver invocation may use, and so the size of
            ...
            trial.
        trial_nodes: Nodes per independent trial job. MPI ranks are split evenly.
        mem_per_task: Memory per trial node. Separate from ``mem_per_node``
            because Ray uses its own allocation.
    ...
    def __post_init__(self) -> None:
        ...
        if self.trial_nodes < 1 or self.solver_cores % self.trial_nodes:
            msg = "solver_cores must be divisible by positive trial_nodes"
            raise ValueError(msg)
        if self.solver_cores // self.trial_nodes > self.cores_per_node:
            msg = (
                f"solver_cores={self.solver_cores} across {self.trial_nodes} nodes "
                f"exceeds cores_per_node={self.cores_per_node}: no trial could ever run"
            )
            raise ValueError(msg)
    ...
    def _write(self, path: str | Path, text: str) -> Path:
        ...
        Path(self.log_dir).resolve().mkdir(parents=True, exist_ok=True)
    ...
    def sbatch_script(self, driver: str) -> str:
        ...
        Raises:
            ValueError: The Ray worker needs more cores than one node provides.
        """
        if self.solver_cores > self.cores_per_node:
            msg = (
                "Ray keeps a trial on one node, so solver_cores exceeds cores_per_node"
            )
            raise ValueError(msg)
...
def submit(script: str | Path, *args: str) -> str:
    ...
    if match is None:
        msg = f"sbatch returned no job id for {script}: {result.stdout.strip()}"
        raise SlurmJobError(msg)
