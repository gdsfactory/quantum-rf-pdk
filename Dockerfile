...
# JupyterLab is only needed by this image, so it is pinned here instead of in the docs
# group. Installing it into the venv (not as a uv tool) keeps the notebook kernel on the
# interpreter that has qpdk; `uv sync` prunes it, so pass --inexact if that matters.
RUN --mount=type=cache,uid=${NB_UID},gid=${NB_UID},target=${HOME}/.cache/uv \
    uv pip install --python ${HOME}/.venv/bin/python jupyterlab==4.6.3
