...
if [ "$#" -gt 0 ]; then
    ...
    for py_file in "$@"; do
        if [ -f "$py_file" ]; then
            ...
            show_changes_output=$( (uvx jupytext --update --to ipynb "$py_file" --output "$ipynb_path" --show-changes || jupytext --update --to ipynb "$py_file" --output "$ipynb_path" --show-changes) 2>&1)
