...
- :doc:`notebooks/palace_batched_qubit_optimization` — Batched geometry optimisation of
  a double-pad transmon, sweeping the pads against a target qubit frequency, the
  relaxation time implied by the energy participation ratios, and the pad footprint,
  with Optuna driving Palace eigenmode solves as Slurm job arrays.
